...
MPIRUN="${MPIRUN:-}"
if [[ -z "$MPIRUN" ]] && command -v spack >/dev/null 2>&1; then
    OMPI_PREFIX="$(spack location -i openmpi@4.1.8 2>/dev/null || true)"
    if [[ -n "$OMPI_PREFIX" && -x "$OMPI_PREFIX/bin/mpirun" ]]; then
        MPIRUN="$OMPI_PREFIX/bin/mpirun"
    fi
fi
[[ -z "$MPIRUN" ]] && MPIRUN="$(command -v mpirun || true)"
[[ -x "${MPIRUN:-/nonexistent}" ]] || { echo "Open MPI 'mpirun' not found. Set MPIRUN=... or 'spack load openmpi'."; exit 1; }
