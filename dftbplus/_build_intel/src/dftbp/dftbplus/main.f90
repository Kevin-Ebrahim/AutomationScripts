...
contains
  ...
  call errStatus%setError(-1, "Dense block Mulliken routine not implemented for MPI parallel build.",&
      & "src/dftbp/dftbplus/main.F90", 4841)
  return
          do iSpin = 1, nSpin
  ...
  call errStatus%setError(-1, "Alternative force evaluation methods are not supported by this electronic solver.",&
      & "src/dftbp/dftbplus/main.F90", 6013)
  return
      end if
  ...
  call errStatus%setError(-1, "The OnlyTransport solver cannot calculate the energy weighted density matrix",&
      & "src/dftbp/dftbplus/main.F90", 6027)
  return
  ...
  if (errStatus%hasError()) then
    ...
    case (electronicSolverTypes%omm, electronicSolverTypes%pexsi, electronicSolverTypes%ntpoly,&
      ...
      if (forceType /= forceTypes%orig) then
  call errStatus%setError(-1, "Alternative force evaluation methods are not supported by this electronic solver.",&
      & "src/dftbp/dftbplus/main.F90", 6044)
  return
      end if
  ...
  subroutine getEDensityMtxFromRealEigvecs(env, denseDesc, forceType, filling, eigen,&
    ...
    do iKS = 1, parallelKS%nLocalKS
      ...
      case(forceTypes%dynamicTFinite)
        ...
        if (info /= 0) then
  call errStatus%setError(info, "XLBOMD failure with real posv non-zero info", "src/dftbp/dftbplus/main.F90", 6328)
  return
        end if
  ...
  subroutine getEDensityMtxFromComplexEigvecs(env, denseDesc, forceType, filling, eigen, kPoint,&
    ...
    do iKS = 1, parallelKS%nLocalKS
      ...
      case(forceTypes%dynamicTFinite)
        ...
        call scalafx_pposv(work, denseDesc%blacsOrbSqr, work2, denseDesc%blacsOrbSqr, info=info)
        if (info /= 0) then
  call errStatus%setError(info, "XLBOMD failure with complex posv non-zero info", "src/dftbp/dftbplus/main.F90", 6562)
  return
        end if
        ! Symmetrize just in case
        work(:,:) = work2
  ...
  call errStatus%setError(-1, "Alternative force evaluation methods are not supported for Pauli hamiltonians at the moment.",&
      & "src/dftbp/dftbplus/main.F90", 6702)
  return
    end if
  ...
  subroutine getGradients(env, parallelKS, boundaryConds, sccCalc, tblite, isExtField, isXlbomd,&
    ...
    if (.not. (tSccCalc .or. isExtField)) then
      ...
      if (tExtChrg) then
        chrgForces(:,:) = 0.0_dp
        if (isXlbomd) then
  call errStatus%setError(-1, "XLBOMD does not work with external charges yet!", "src/dftbp/dftbplus/main.F90", 6971)
  return
        else
    ...
    if (allocated(solvation)) then
      if (isXlbomd) then
  call errStatus%setError(-1, "XLBOMD does not work with solvation yet!", "src/dftbp/dftbplus/main.F90", 7017)
  return
      else
        if (areSolventNeighboursSym) then
  ...
  call errStatus%setError(-1, "Not implemented yet!", "src/dftbp/dftbplus/main.F90", 7072)
  return
        end if
  if (errStatus%hasError()) then
    return
