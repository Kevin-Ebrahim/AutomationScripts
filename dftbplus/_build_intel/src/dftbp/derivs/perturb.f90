...
contains
  ...
  call errStatus%setError(-1, "Onsite corrected and DFTB+U terms currently not compatible for perturbation",&
      & "src/dftbp/derivs/perturb.F90", 750)
  return
    end if
    if (allocated(dftbU) .or. allocated(onsMEs)) then
      allocate(dqBlockIn(orb%mOrb,orb%mOrb,nAtom,nSpin))
  ...
  call errStatus%setError(-1, "SCC in perturbation is NOT converged, maximal SCC iterations exceeded",&
      & "src/dftbp/derivs/perturb.F90", 1514)
  return
      else
  ...
  call errStatus%setError(-1, "Atom derivative perturbations are not currently supported for metallic systems",&
      & "src/dftbp/derivs/perturb.F90", 2066)
  return
    end if
  ...
  call errStatus%setError(-1, "Atom derivative perturbations are not currently supported for high-symmetry/degenerate systems",&
      & "src/dftbp/derivs/perturb.F90", 2074)
  return
      end if
    end do
    ...
    if (allocated(hybridXc)) then
  call errStatus%setError(-1, "Range separation not supported for MPI at the moment", "src/dftbp/derivs/perturb.F90", 2161)
  return
      allocate(SSqrReal(nOrbs, nOrbs))
      SSqrReal(:,:) = 0.0_dp
...
!                  & dEi, dPsiCmplx, iAt)
  ...
  call errStatus%setError(-1, "Shouldn't be here in dxAtom routine", "src/dftbp/derivs/perturb.F90", 2394)
  return
