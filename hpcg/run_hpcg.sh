...
ARCH="${ARCH:-openblas_ompi_gcc}" # using same default arch name as HPL
NP="${NP:-$(nproc)}"
...
# Check for HPCG.dat (should be present by default after build)
if [[ ! -f "$HPCG_DAT" ]]; then
    echo "Warning: $HPCG_DAT not found. Using default HPCG settings; ensure HPCG.dat is present in $BIN_DIR." >&2
fi

# Assemble mpirun command
MPI_CMD=(mpirun ${HOSTFILE:+--hostfile "$HOSTFILE"} -np "$NP" "$HPCG_BIN")
